...
class IGEVStereo(nn.Module):
    def __init__(self, args):
        ...
        # VDFNet: volume rendering replaces soft-argmin
        D = args.max_disp // 4
        self.render = disparityrender(0, D - 1, D)
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision, dtype=getattr(torch, self.args.precision_dtype, torch.float16)):
            ...
            # Init disp from geometry encoding volume via volume rendering
            density = self.classifier(geo_encoding_volume).squeeze(1) * self.density_temperature  # [B, D, H, W]
            init_disp = self.render(density)  # [B, 1, H, W], range [0, D-1]
