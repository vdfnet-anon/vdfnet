...
class disparityrender(nn.Module):
    ...
    Treats cost volume as density field, applies NeRF-style alpha compositing."""
    ...
    def forward(self, x):
        ...
        T = torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1, alpha.shape[2], alpha.shape[3]), device=alpha.device), 1. - alpha + 1e-10], 1), 1)[:, :-1, :, :]
        weights = alpha * T
