    ...
    def __call__(self, sample):
        ...
        if self.img_height > ori_height or self.img_width > ori_width:
            ...
            sample['right'] = np.pad(sample['right'],
                                         ...
                                         constant_values=0)
            if 'disp' in sample.keys():
                sample['disp'] = np.pad(sample['disp'],
                                            ((top_pad, 0), (0, right_pad)),
                                            mode='constant',
                                            constant_values=0)

            if 'pseudo_disp' in sample.keys():
                sample['pseudo_disp'] = np.pad(sample['pseudo_disp'],
                                                   ((top_pad, 0), (0, right_pad)),
                                                   mode='constant',
                                                   constant_values=0)
...
def _adjust_img(img, func):
    return np.clip(func(img.astype(np.float32)), 0, 255).astype(np.uint8)
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            for key in ['left', 'right']:
                ...
                sample[key] = np.clip(mean + (img - mean) * factor, 0, 255).astype(np.uint8)
        return sample
...
class RandomGamma(object):
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            for key in ['left', 'right']:
                sample[key] = cv2.LUT(sample[key], table)
        return sample
...
class RandomBrightness(object):
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            for key in ['left', 'right']:
                sample[key] = np.clip(sample[key].astype(np.float32) * factor, 0, 255).astype(np.uint8)
        return sample
...
class RandomHue(object):
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            hue_shift = np.random.uniform(-0.1, 0.1) * 180
            for key in ['left', 'right']:
                hsv = cv2.cvtColor(sample[key], cv2.COLOR_RGB2HSV).astype(np.float32)
                hsv[:, :, 0] = (hsv[:, :, 0] + hue_shift) % 180
                sample[key] = cv2.cvtColor(hsv.astype(np.uint8), cv2.COLOR_HSV2RGB)
        return sample
