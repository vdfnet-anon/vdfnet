...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root=None, dstype='frames_finalpass', things_test=False):
        super(SceneFlowDatasets, self).__init__(aug_params)
        if root is None:
            root = os.environ.get('SCENEFLOW_DIR', '/data/sceneflow/')
        self.root = root
        self.dstype = dstype
    ...
    def _add_things(self, split='TRAIN'):
        ...
        logging.info(f"Added {len(self.disparity_list) - original_length} from FlyingThings3D {self.dstype}")
