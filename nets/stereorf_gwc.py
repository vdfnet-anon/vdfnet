...
class stereorf_gwc(nn.Module):
    """VDFNet with Group-wise Correlation (GWC) cost volume.

    Replaces the concat volume (C*2=64 channels) with GWC (num_groups=32 channels),
    matching the paper description and improving matching discriminability.
    """

    def __init__(self, arg):
        super(stereorf_gwc, self).__init__()

        self.maxdisp = 192
        self.num_groups = 32  # channels_per_group = 32/32 = 1, empirically optimal

        self.feature_extraction = feature_extraction()

        # GWC produces num_groups channels (vs concat's C*2=64)
        ...
        self.fineserchr = 8  # widen the fine-stage search range (original = 4)

        self.classif1 = nn.Sequential(convbn_3d(32, 32, 3, 1, 1),
                                      nn.ReLU(inplace=True),
                                      nn.ConvTranspose3d(32, 1, 4, 4, 0, bias=True),
                                      nn.ReLU(inplace=True))
    ...
    def build_gwc_volume(self, refimg_fea, targetimg_fea):
        """Group-wise correlation cost volume with L2 normalization.

        Splits features into num_groups groups and computes normalized inner product
        within each group across all disparity levels.
        """
        B, C, H, W = refimg_fea.size()
        D = self.maxdisp // 4
        G = self.num_groups
        assert C % G == 0, f"Feature channels {C} must be divisible by num_groups {G}"
        channels_per_group = C // G

        # [B, G, channels_per_group, H, W]
        ref = refimg_fea.view(B, G, channels_per_group, H, W)
        tgt = targetimg_fea.view(B, G, channels_per_group, H, W)

        cost = torch.zeros(B, G, D, H, W, device=refimg_fea.device, dtype=refimg_fea.dtype)
        # d=0: full overlap
        cost[:, :, 0, :, :] = (ref * tgt).sum(dim=2)
        for i in range(1, D):
            cost[:, :, i, :, i:] = (ref[:, :, :, :, i:] * tgt[:, :, :, :, :-i]).sum(dim=2)
    ...
    def forward(self, left, right):
        ...
        # GWC cost volume
        cost = self.build_gwc_volume(refimg_fea, targetimg_fea)
        ...
        if self.training:
            ...
            pred3 = (disparityrender(-self.fineserchr, self.fineserchr, 2 * self.fineserchr + 1)(rf3.squeeze(1)).squeeze(1)
                     + disparityrender(1, 1, 2 * self.fineserchr + 1)(rf3.squeeze(1)).squeeze(1)
                     * pred2[:, ty:ty + sh, tx:tx + sw].detach())
        else:
            rf1 = self.classif1(out1)
            pred1 = disparityrender(0, self.maxdisp, self.maxdisp)(rf1.squeeze(1))
            rf2 = self.classif2(out2)
            ...
            rf3 = self.classif3(candibase, wrapcost)
            pred3 = (disparityrender(-self.fineserchr, self.fineserchr, 2 * self.fineserchr + 1)(rf3.squeeze(1)).squeeze(1)
                     + disparityrender(1, 1, 2 * self.fineserchr + 1)(rf3.squeeze(1)).squeeze(1) * pred2)
